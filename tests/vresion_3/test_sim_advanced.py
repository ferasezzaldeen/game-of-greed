...
def test_repeat_roller():
    ...
    """
    diffs = diff(Game().play, path="tests/vresion_3/repeat_roller.sim.txt")
...
def test_hot_dice():
    ...
    """
    diffs = diff(Game().play, path="tests/vresion_3/hot_dice.sim.txt")
    assert not diffs, diffs
...
def test_cheat_and_fix():
    ...
    diffs = diff(Game().play, path="tests/vresion_3/cheat_and_fix.sim.txt")
    assert not diffs, diffs
...
def test_zilcher():
    ...
    diffs = diff(Game().play, path="tests/vresion_3/zilcher.sim.txt")
    assert not diffs, diffs